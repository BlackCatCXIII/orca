...
`schema.json` is the canonical machine-readable contract for the JSON envelope Orca writes to suspend, resume, and destroy recipe stdin. `recipeResult` remains opaque because its result schema is independently versioned; consumers must not treat that nested result as the lifecycle payload itself.
